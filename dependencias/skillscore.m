...
%
function [SKout, metric_id, R2, RMSE] = skillscore(obsData, simData)
...
for i=1:size(obsData,1)
    [~,loc_sim(i)]=min(abs(simData(:,1)-obsData(i,1)));
end
...
MatchedData = [obsData(:,1) obsData(:,2) simData(loc_sim,2)];
...
aux = corrcoef(MatchedData(:,2),MatchedData(:,3));
R2=aux(2)^2;
if r < 10
